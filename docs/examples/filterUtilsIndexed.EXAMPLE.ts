...
export function filterEventsIndexed(
  ...
  if (filters.mapBounds && indexes.spatial) {
    ...
    console.log(`Viewport filter: ${candidates.length} events in view`);
  }
  ...
  if (filters.selectedHazards.length > 0) {
    ...
    console.log(`Hazard filter: ${candidates.length} events remain`);
  }
...
export function aggregateEventsByLevelIndexed(
  ...
  if (level === 'national') {
    ...
    return [{
      ...
      totalAffectedPopulation: events.reduce((sum, e) => sum + (e.totalAffectedPopulation || 0), 0),
      totalEconomicDamage: events.reduce((sum, e) => sum + (e.totalEconomicDamage || 0), 0),
  ...
  events.forEach(event => {
    ...
    // Skip events without a valid key
    if (!key) return;
    ...
    metrics.totalAffectedPopulation += event.totalAffectedPopulation || 0;
    metrics.totalEconomicDamage += event.totalEconomicDamage || 0;
...
export function getDistrictStatsIndexed(
  ...
  return {
    ...
    totalDamage: districtEvents.reduce((sum, e) => sum + (e.totalEconomicDamage || 0), 0),
    totalPopulation: districtEvents.reduce((sum, e) => sum + (e.totalAffectedPopulation || 0), 0),
    primaryHazard,
  };
}
