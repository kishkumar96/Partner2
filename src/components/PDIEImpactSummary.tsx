...
export default function PDIEImpactSummary({
  ...
  nationalSummary = [],
  isLoading = false,
}: PDIEImpactSummaryProps) {
  if (isLoading) {
    return (
      <div className="glass-panel rounded-xl p-6 animate-fadeSlide">
        <h3 className="text-lg font-semibold text-slate-100 mb-4">Loading Impact Data...</h3>
      </div>
    );
  }

  const totalLoss = impactBySector.reduce(
    (sum, row) => sum + Number(row.Total_Loss || 0),
    0
  );
  const totalExposed = Number(
    nationalSummary?.[0]?.Buildings_Exposed_To_Any_Hazard || 0
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="glass-panel glass-panel-hover rounded-xl p-6 border-l-4 border-neon-coral">
          <h3 className="text-sm font-semibold tracking-widest text-slate-300 uppercase">Total Economic Loss</h3>
        ...
        <div className="glass-panel glass-panel-hover rounded-xl p-6 border-l-4 border-neon-amber">
          <h3 className="text-sm font-semibold tracking-widest text-slate-300 uppercase">Buildings Exposed</h3>
        ...
        <div className="glass-panel glass-panel-hover rounded-xl p-6 border-l-4 border-neon-purple">
          <h3 className="text-sm font-semibold tracking-widest text-slate-300 uppercase">Buildings Damaged</h3>
      ...
      <div className="glass-panel rounded-xl p-6 animate-fadeSlide">
        ...
        <p className="text-sm text-slate-300">
          The Impact by Sector and Impact by Asset Type tables are now permanently visible in the right sidebar for quick reference.
        </p>
      </div>
    </div>
  );
