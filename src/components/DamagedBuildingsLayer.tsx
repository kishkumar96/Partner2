...
export default function DamagedBuildingsLayer({
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MapMouseEvent & { features?: maplibregl.MapGeoJSONFeature[] }) => {
      ...
      damageP.appendChild(
        document.createTextNode(` $${Number(props.Wind_Loss || 0).toLocaleString()}`)
      );
      ...
      exposureP.appendChild(
        document.createTextNode(` $${Number(props.Exposure || 0).toLocaleString()}`)
      );
      ...
      damageRatioP.appendChild(
        document.createTextNode(` ${((Number(props.Damage_Ratio) || 0) * 100).toFixed(1)}%`)
      );
      ...
      const buildingTypeText = props.Building_Type || props.BTypeCat || 'Unknown';
      buildingTypeP.appendChild(document.createTextNode(` ${buildingTypeText}`));
      ...
      occupancyP.appendChild(document.createTextNode(` ${occupancyText}`));
        ...
        map.addLayer(
          {
            ...
            paint: {
              ...
              'circle-radius': [
                ...
                100,
                28, // radius for clusters with >= 100 points (increased from 24)
                750,
                35, // radius for clusters with >= 750 points (increased from 30)
              ],
              ...
              'circle-color': [
                ...
                100,
                BUILDING_DAMAGE_COLORS.substantial, // orange for medium clusters
                750,
                BUILDING_DAMAGE_COLORS.severe, // red for large clusters
              ],
              // Reduced opacity to prevent overwhelming regional fills and basemap
              'circle-opacity': 0.85,
              'circle-stroke-width': 4, // Very thick stroke (increased from 3)
              'circle-stroke-color': '#ffffff',
              // Reduced stroke opacity for better visual hierarchy
        ...
        map.addLayer(
          {
            ...
            paint: {
              ...
              'circle-stroke-color': '#ffffff',
              // Softer stroke for less visual competition
              'circle-stroke-opacity': 0.85,
            },
          },
          firstSymbolId
